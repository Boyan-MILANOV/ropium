...
ROPChain* ROPCompiler::_set_registers_permutation( vector<ILInstruction>& instr, vector<int>& permutation, Constraint* constraint, list<vector<int>>& failed_perms){
    ...
    for( int i = 0; i < permutation.size(); i++ ){
        ...
        if( chain == nullptr ){
            ...
            for( int j = i+1; j < permutation.size(); j++ ){
                if( instr[j].type == ILInstructionType::MOV_REG && 
                    std::find(tmp_perm.begin(), tmp_perm.end(), instr[j].args[PARAM_MOVREG_SRC_REG]) == tmp_perm.end())
                    tmp_perm.push_back(instr[j].args[PARAM_MOVREG_SRC_REG]);
            }
...
ROPChain* ROPCompiler::_set_multiple_registers(vector<ILInstruction>& instr, Constraint* constraint){
    ...
    do{
        if( !_is_failed_permutation(failed_permutations, order)){
            res = _set_registers_permutation(instr, order, constraint, failed_permutations);
            if( res ){
